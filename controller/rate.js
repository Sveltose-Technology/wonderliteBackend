...
exports.addrate = async (req, res) => {
    const { rate, unit, qtypackage, fordealer, starting_from, closing_on, sortorder, status } = req.body
    ...
    const findexist = await Rate.findOne({ $and: [{ rate: rate }, { qtypackage: qtypackage }, { fordealer: fordealer },] })
    if (findexist) {
        res.status(400).json({
            status: false,
            msg: "Already Exists",
...
exports.editrate = async (req, res) => {
    const findandUpdateEntry = await Rate.findOneAndUpdate({
        _id: req.params.id
    }, { $set: req.body }, { new: true }).populate('unit')
